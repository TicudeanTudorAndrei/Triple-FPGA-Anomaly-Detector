...
constant MEAN_ID_DEPTH : integer := 16;
type mean_id_fifo_t is array(0 to MEAN_ID_DEPTH-1) of integer range 0 to NUM_STREAMS-1;
signal mean_id_fifo : mean_id_fifo_t := (others => 0);
signal mean_id_wr   : integer range 0 to MEAN_ID_DEPTH-1 := 0;
signal mean_id_rd   : integer range 0 to MEAN_ID_DEPTH-1 := 0;
...
-- Selector for sum FIFOs into fp_div
process(aclk)
begin
    if rising_edge(aclk) then
        if aresetn = '0' then
            ...
            if fifo_m_tvalid(fifo_select) = '1' and div_ready_a = '1' then
                ...
                mean_id_fifo(mean_id_wr) <= fifo_select;
                if mean_id_wr = MEAN_ID_DEPTH - 1 then mean_id_wr <= 0;
                else mean_id_wr <= mean_id_wr + 1; end if;
            end if;
...
begin
    if rising_edge(aclk) then
        if aresetn = '0' then
            ...
            if mean_tvalid_signal = '1' and mean_tready = '1' then
                if mean_id_rd = MEAN_ID_DEPTH - 1 then mean_id_rd <= 0;
                else mean_id_rd <= mean_id_rd + 1; end if;
            end if;
        end if;
    end if;
end process;

mean_id_signal <= std_logic_vector(to_unsigned(mean_id_fifo(mean_id_rd), 16));
