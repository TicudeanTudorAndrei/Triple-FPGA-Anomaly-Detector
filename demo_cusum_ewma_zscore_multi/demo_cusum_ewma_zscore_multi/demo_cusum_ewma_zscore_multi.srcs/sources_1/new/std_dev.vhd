...
constant STD_ID_DEPTH : integer := 64;
type std_id_fifo_t is array(0 to STD_ID_DEPTH-1) of integer range 0 to NUM_STREAMS-1;
signal std_id_fifo : std_id_fifo_t := (others => 0);
signal std_id_wr   : integer range 0 to STD_ID_DEPTH-1 := 0;
signal std_id_rd   : integer range 0 to STD_ID_DEPTH-1 := 0;
...
-- push sqrd_dev stream ID in fp_div
process(aclk)
begin
    if rising_edge(aclk) then
        if aresetn = '0' then
            ...
            if sqrddev_tvalid = '1' and div_a_tready = '1' then
                std_id_fifo(std_id_wr) <= conv_integer(sqrd_dev_id);
                if std_id_wr = STD_ID_DEPTH - 1 then std_id_wr <= 0;
                else std_id_wr <= std_id_wr + 1; end if;
            end if;

            if std_dev_tvalid_signal = '1' and std_dev_tready = '1' then
                if std_id_rd = STD_ID_DEPTH - 1 then std_id_rd <= 0;
                else std_id_rd <= std_id_rd + 1; end if;
            end if;
        end if;
    end if;
end process;

std_dev_id_signal <= std_logic_vector(to_unsigned(std_id_fifo(std_id_rd), 16));
